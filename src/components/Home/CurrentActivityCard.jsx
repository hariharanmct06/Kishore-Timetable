...
import { Play, Pause, CheckCircle2, Maximize2, Zap, Clock } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { formatSecondsToHHMMSS, formatTime24To12, formatClockTime, formatHeaderDate } from '../../utils/timeEngine';
...
export function CurrentActivityCard() {
  ...
  const { activeSlot, nextSlot, remainingSeconds, dayBlueprint } = timeState;
  ...
  if (activeSlot.type === 'study') {
    if (activeSlot.sessionIndex === 1) subjectName = dayBlueprint?.subjects[0] || "Physics";
    else if (activeSlot.sessionIndex === 2) subjectName = dayBlueprint?.subjects[1] || "Maths";
    else if (activeSlot.sessionIndex === 3) subjectName = dayBlueprint?.subjects[2] || "Chemistry";
    else if (activeSlot.sessionIndex === 4) subjectName = dayBlueprint?.subjects[0] || "Physics";
    else if (activeSlot.sessionIndex === 5) subjectName = dayBlueprint?.subjects[1] || "PYQs";
    ...
    <div className="space-y-3">
      ...
      <div className="md:hidden space-y-1 py-1">
        ...
        <div className="flex items-center justify-between text-xs text-slate-400">
          <p className="font-semibold text-indigo-300">JEE Main Command Center</p>
          <span className="font-mono text-slate-400">{formatHeaderDate(timeState.now).split(',')[0]} • {formatClockTime(timeState.now)}</span>
        </div>
      </div>
      ...
      <div className={`p-5 sm:p-6 rounded-3xl border transition-all duration-300 shadow-xl relative overflow-hidden ${
        ...
        <div className="flex flex-col space-y-4">
          ...
          {/* Timestamp-Based Live Countdown */}
          <div className="py-2.5 text-center bg-slate-950/60 rounded-2xl border border-slate-800/80">
