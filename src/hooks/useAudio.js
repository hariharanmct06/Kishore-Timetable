  ...
  const playChime = (type = 'start') => {
    try {
      ...
      if (type === 'start') {
        ...
        playTone(ctx, 783.99, 0.3, 0.4); // G5
      } else if (type === 'complete') {
        playTone(ctx, 587.33, 0, 0.15); // D5
        playTone(ctx, 739.99, 0.15, 0.15); // F#5
        playTone(ctx, 880.00, 0.3, 0.15); // A5
        playTone(ctx, 1174.66, 0.45, 0.5); // D6
      } else if (type === 'warning') {
        playTone(ctx, 440, 0, 0.2); // A4
        playTone(ctx, 440, 0.25, 0.2); // A4
      } else if (type === 'break') {
        playTone(ctx, 659.25, 0, 0.2); // E5
        playTone(ctx, 523.25, 0.2, 0.4); // C5
      }
    } catch (e) {
